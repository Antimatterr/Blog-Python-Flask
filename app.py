...
# to use mysql database install mysqlclient from unofficial binary site download whl file!
...
def contact():
    if request.method == 'POST':
        name = request.form.get('name')  # add data to database
        ...
        db.session.commit()
        mail.send_message('New message on Blog WiSe PAge from' + name,
                          sender=email,
